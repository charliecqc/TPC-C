...
/*global variable for profiling */
unsigned long long total_writer_op_time, total_writer_op_count;
...
int main( int argc, char *argv[] )
{
  ...
  struct timespec total_writer_op_time_local[2];

  // start time
  clock_gettime(CLOCK_MONOTONIC, &total_writer_op_time_local[0]); 
...
*/
  ...
  // finish time
  clock_gettime(CLOCK_MONOTONIC, &total_writer_op_time_local[1]);

  // calc
  app_calclock(total_writer_op_time_local, &total_writer_op_time, &total_writer_op_count);
